...
int main(int argc, char **argv){
  ...
  for (int ii=0; ii<=STEPS; ++ii){
    evolve(data);
    plot_gnuplot(data); // para que grafique hacemos ./a.out | gnuplot
    // Es decir, lo entubamos a gnuplot
  }
  return 0;
}
...
void plot_gnuplot(const Matrix & m){ // '-' Hace que gnuplot espere los datos
